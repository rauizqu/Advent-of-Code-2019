...
public class Dia3_Parte2 {
	...
	public static void main(String[] args) {
		...
		try {
	          ...
	          ArrayList<Point> puntos_interseccion = new ArrayList<Point>();	
	          Point punto_interseccion;
	          int min=10000000;
	          ...
	          for (int j=0;j<puntos_1.size();j++) {
	        	  for(int k=0;k<puntos_2.size();k++) {
		        	  if(puntos_1.get(j).equals(puntos_2.get(k))&&(Math.abs(puntos_1.get(j).x)+Math.abs(puntos_1.get(j).y)!=2)){
		        		  ...
		        		  punto_interseccion=new Point(puntos_1.get(j).x,puntos_1.get(j).y);
		        		  puntos_interseccion.add(punto_interseccion);
	          ...
	          int suma_final;
	          ArrayList<Integer> array_suma_final = new ArrayList<Integer>();
	          int j;
	          int j_def=0;
	          for(int l=0;l<puntos_interseccion.size();l++) {
		          for (j=0;j<puntos_1.size();j++) {
		          	  if(puntos_interseccion.get(l).equals(puntos_1.get(j))) {
		          		  j_def=j;
		          		  break;
		          	  }
		          }
		          int k;
		          int k_def=0;
		          for (k=0;k<puntos_2.size();k++) {
		          	  if(puntos_interseccion.get(l).equals(puntos_2.get(k))) {
		          		  k_def=k;
		          		  break;
		          	  }
		          }
		          suma_final=j_def+k_def;
		          array_suma_final.add(suma_final);
	          }
	          int minimo_absoluto=10000000;
	          for(int m=0;m<array_suma_final.size();m++) {
	        	  if(array_suma_final.get(m)<minimo_absoluto) {
	        		  minimo_absoluto=array_suma_final.get(m);
	        	  }
	          }
	          System.out.println(minimo_absoluto);
